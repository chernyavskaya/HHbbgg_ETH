#env setup for t3
###export PATH=/swshare/anaconda/bin:$PATH
#source /afs/cern.ch/sw/lcg/external/gcc/4.9/x86_64-slc6-gcc49-opt/setup.sh
#source /afs/cern.ch/sw/lcg/app/releases/ROOT/6.05.02/x86_64-slc6-gcc49-opt/root/bin/thisroot.sh
###source /cvmfs/sft.cern.ch/lcg/releases/LCG_94/ROOT/6.14.04/x86_64-slc6-gcc62-opt/ROOT-env.sh
export PATH=/swshare/anaconda/bin:$PATH
source /cvmfs/sft.cern.ch/lcg/releases/LCG_84/gcc/4.9.3/x86_64-slc6/setup.sh
source /cvmfs/sft.cern.ch/lcg/releases/LCG_84/ROOT/6.06.02/x86_64-slc6-gcc49-opt/bin/thisroot.sh
export PYTHONPATH=/swshare/python/anaconda/lib/$PYTHONPATH
#source /cvmfs/cms.cern.ch/slc6_amd64_gcc493/external/gcc/4.9.3/etc/profile.d/init.sh
#source /cvmfs/cms.cern.ch/slc6_amd64_gcc493/lcg/root/6.06.04/bin/thisroot.sh
#export PYTHONPATH=$PYTHONPATH:~/HHbbgg_ETH/Training/python/
#export PYTHONPATH=$PYTHONPATH:~/HHbbgg_ETH_devel/Training/python/
#export PYTHONPATH=$PYTHONPATH:~/HHbbgg_ETH_devel/bregression/python/
#export PYTHONPATH=/mnt/t3nfs01/data01/shome/nchernya/xgboost/python-package/:/afs/cern.ch/sw/lcg/app/releases/ROOT/6.05.02/x86_64-slc6-gcc49-opt/root/lib/:~/HHbbgg_ETH_devel/bregression/python/
export PYTHONPATH=/mnt/t3nfs01/data01/shome/nchernya/xgboost/python-package/:/afs/cern.ch/sw/lcg/app/releases/ROOT/6.05.02/x86_64-slc6-gcc49-opt/root/lib/
#export PYTHONPATH=$PYTHONPATH:~/HHbbgg_ETH_devel/Training/python/
#export PYTHONPATH=$PYTHONPATH:~/HHbbgg_ETH_devel/ConversionUtils/python/
export PYTHONPATH=$PYTHONPATH:/cvmfs/sft.cern.ch/lcg/releases/LCG_84/ROOT/6.06.02/x86_64-slc6-gcc49-opt/lib/
export PYTHONPATH=$PYTHONPATH:/shome/nchernya/HHbbgg_ETH_devel/Training/python/
export PYTHONPATH=$PYTHONPATH:/shome/nchernya/HHbbgg_ETH_devel/ConversionUtils/python/
